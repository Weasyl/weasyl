...
if (tagEditType !== 'none') {
    ...
    if (!tagActions || !tagActions.classList.contains('tag-actions')) {
    ...
    if (tagActions) { tagActions.insertAdjacentElement('afterbegin', editButton); }
    ...
    if (tagEditType === 'edit') {
        ...
        byClass('suggested-tags', manage).addEventListener('click', (e) => {
            ...
            const {tagAction} = target.dataset;

            if (!tagAction) {
                return;
            }

            const actionsFieldset = target.parentNode;
    if (!actionsFieldset) return;
            const tag = actionsFieldset.parentNode?.dataset; const {tag} = tag;
