...
def test_DMY_not_integer_raises_birthdayInvalid_WeasylError():
    ...
    form = web.Storage(username=user_name, password='', passcheck='',
...
def test_passwords_must_match():
    ...
    form = web.Storage(username=user_name, password='123', passcheck='qwe',
                       email='test@weasyl.com', emailcheck='test@weasyl.com',
...
def test_passwords_must_be_of_sufficient_length():
    password = "tooShort"
    form = web.Storage(username=user_name, password=password, passcheck=password,
...
def test_username_cant_be_blank_or_have_semicolon():
    form = web.Storage(username='...', password='0123456789', passcheck='0123456789',
                       email=email_addr, emailcheck=email_addr,
                       day='12', month='12', year=arrow.now().year - 19)
    with pytest.raises(WeasylError) as err:
        login.create(form)
...
def test_create_fails_if_username_is_a_prohibited_name():
    form = web.Storage(username='testloginsuite', password='0123456789', passcheck='0123456789',
                       email='test@weasyl.com', emailcheck='test@weasyl.com',
                       day='12', month='12', year=arrow.now().year - 19)
