...
def select_profile(userid, viewer=None):
    query = d.engine.execute("""
        SELECT pr.username, pr.full_name, pr.catchphrase, pr.timestamp, pr.profile_text,
...
def select_manage(userid):
    ...
    query = d.engine.execute("""
        SELECT
            ...
            pr.timestamp, pr.username, pr.full_name, pr.catchphrase,
...
def do_manage(my_userid, userid, username=None, full_name=None, catchphrase=None,
    ...
    if birthday is not None:
        ...
        if len(split) != 3:
            try:
                unixtime = datetime(day=int(split[2]), month=int(split[1]), year=int(split[0]))
            except ValueError:
