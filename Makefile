...
# Staff
config/weasyl-staff.json:
	cp -n config/weasyl-staff.json.example $@
...
# Phony setup target
.PHONY: setup
setup: $(VE) config/site.config.txt config/weasyl-staff.json build/rev-manifest.json $(STATIC_DIRS) $(TEMP_DIRS)
