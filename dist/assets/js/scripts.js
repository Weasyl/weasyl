...
const reportInputChanged = () => {
    ...
    if (action === 'action_taken') {
        ...
        userNoteBox.show();
        reportButtons.prop('disabled', !noteTitle.val() || !closureExplanation.val() || !userNote.val());
        reportClosureWarning.text('The user note will be sent to the reported user with the ' +
        ...
        reportClosureWarning.text('The report closure explanation entered will be visible to ' +
...
const canTriggerShortcut = e => !['INPUT', 'SELECT', 'TEXTAREA'].includes(e.target.nodeName);
...
{
    const isOtherOption = option => option.hasAttribute('data-select-other');
