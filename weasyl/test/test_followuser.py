...
class FollowUserTestCase(unittest.TestCase):
    ...
    def test_insert(self, followuser_insert, followuser_remove, get_config):
        ...
        self.assertEqual(False, select_relation(user1, user2)["follow"])
        # user1 watches user2
        followuser.insert(user1, user2)
        self.assertEqual(1, d.sessionmaker().query(orm.Follow).count())
        # select_relation should return u1 follows u2, u2 is follower of u1, u1 is not
        # follower of u2
        self.assertEqual(True, select_relation(user1, user2)["follow"])
        self.assertEqual(True, select_relation(user2, user1)["follower"])
        self.assertEqual(False, select_relation(user1, user2)["follower"])
    ...
    def test_insert_ignores(self):
        ...
        self.assertEqual(False, select_relation(user1, user2)["follow"])
        self.assertEqual(False, select_relation(user2, user1)["follower"])
    ...
    def test_mutual_follow(self):
        ...
        self.assertEqual(True, select_relation(user1, user2)["follow"])
        self.assertEqual(True, select_relation(user2, user1)["follower"])
        self.assertEqual(True, select_relation(user2, user1)["follow"])
        self.assertEqual(True, select_relation(user1, user2)["follower"])