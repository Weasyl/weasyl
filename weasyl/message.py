...
def remove_all_before(userid, before):
    d.engine.execute(
        "DELETE FROM welcome WHERE userid = %(user)s AND type = ANY (%(types)s) AND timestamp < %(before)s",
        user=userid, types=list(notification_clusters), before=datetime.utcfromtimestamp(before))
...
def remove_all_submissions(userid, only_before=None):
    if not only_before:
        only_before = datetime.now

    d.engine.execute(
        "DELETE FROM welcome WHERE userid = %(user)s AND type IN (2010, 2030, 2040, 2050) AND timestamp < %(before)s",
...
def select_journals(userid):
    journals = d.engine.execute("""
        SELECT we.welcomeid, we.timestamp, we.otherid, we.targetid, pr.username, jo.title
        FROM welcome we
...
def select_submissions(userid, limit, include_tags, backtime=None, nexttime=None):
    if backtime:
        time_filter = "AND we.timestamp > %(backtime)s"
    elif nexttime:
        time_filter = "AND we.timestamp < %(nexttime)s"
    else:
        time_filter = ""
...
def select_site_updates(userid):
    return [{
        ...
        SELECT we.welcomeid, we.timestamp, su.updateid, su.title
        FROM welcome we
...
def select_notifications(userid):
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.type, we.timestamp, we.otherid, pr.username, pr.stream_url
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
        WHERE we.userid = %(user)s
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.type, we.timestamp, we.otherid, we.targetid, pr.username, su.title
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
            INNER JOIN submission su ON we.targetid = su.submitid
        WHERE we.userid = %(user)s
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.timestamp, we.otherid, pr.username
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
        WHERE we.userid = %(user)s
            AND we.type = 3010
    ...
    for t, id_field, title_field, table_name in [(3020, "submitid", "title", "submission"),
        ...
        queries.append([{
            ...
            SELECT
                we.welcomeid, we.timestamp, we.otherid, pr.username, {table}.{id_field}
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.timestamp, su.submitid, su.title
        FROM welcome we
...
def select_comments(userid):
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.timestamp, we.otherid, we.targetid, pr.username
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
        WHERE we.userid = %(user)s
            AND we.type = 4010
    ...
    queries.append({
        ...
        SELECT
            we.welcomeid, we.timestamp, we.otherid, we.referid, we.targetid, pr.username, px.userid AS ownerid,
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.timestamp, we.otherid, we.referid, we.targetid, pr.username, su.title
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.timestamp, we.otherid, we.referid, we.targetid, pr.username, su.submitid, su.title
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.timestamp, we.otherid, we.referid, we.targetid, pr.username, ch.char_name
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.timestamp, we.otherid, we.referid, we.targetid, pr.username, ch.charid, ch.char_name
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.timestamp, we.otherid, we.referid, we.targetid, pr.username, jo.title
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.timestamp, we.otherid, we.referid, we.targetid, pr.username, jo.journalid, jo.title
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.timestamp, we.otherid, we.referid, we.targetid, pr.username, up.title
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
    ...
    queries.append({
        ...
        SELECT we.welcomeid, we.timestamp, we.otherid, we.referid, we.targetid, pr.username, up.updateid, up.title
        FROM welcome we
            INNER JOIN profile pr ON we.otherid = pr.userid
